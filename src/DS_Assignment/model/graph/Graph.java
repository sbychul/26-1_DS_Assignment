...
public class Graph {
    ...
    public Graph() {
        this.adjList = new EnumMap<>(NodeName.class);
        // 11개의 정점 초기화
    ...
    private void initializeEdges() {
        // 1. 스폰 지역 연결
        addEdge(NodeName.SPAWN_A, NodeName.ML);
        addEdge(NodeName.SPAWN_B, NodeName.MR);
    ...
    /**
     * 동적 가중치(위험도) 계산 메서드
     * 간선의 기본 위험도는 1이지만, 목적지 노드에 적 탱커나 DPS가 존재하면 위험도 2가 추가.
     * @param targetNode 진입하려는 목적지 노드
     * @param occupancyMap 현재 모든 유닛의 위치 정보가 담긴 구조 (GameManager에서 제공 예정)
     * @return 최종 계산된 동적 위험도 비용
     */
    public int getDynamicWeight(NodeName targetNode, Map<NodeName, List<Unit>> occupancyMap, Team myTeam) {
        int baseWeight = 1;

        // 목적지 노드에 있는 유닛들 가져오기
        List<Unit> unitsAtNode = occupancyMap.get(targetNode);

        // 노드가 비어있지 않다면 검사
        if (unitsAtNode != null) {
            for (Unit unit : unitsAtNode) {

                // 적군인지 검사
                boolean isEnemy =
                        unit.getTeam() != myTeam;

                // 위험 유닛인지 검사 (Tank 또는 DPS)
                boolean isDangerous =
                        unit instanceof Tank || unit instanceof DPS;

                // 적군 + 위험 유닛 여부 + 생존 여부 만족하면 해당 노드로 향하는 위험도 증가
                if (isEnemy && isDangerous && unit.isAlive()) {
                    baseWeight += 2;
                    break;
                }
            }
